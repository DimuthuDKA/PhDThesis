...
n=54; % # of trajs
nT=1; % # times to do the same fit
ttraj=linspace(0,1/minf,n+1);
ttraj(end)=[];
...
for i=1:n
    ...
    tx=ttraj(i);
    ty=Aa*sin(2*pi*fa*tx);
    tz=Ab*sin(2*pi*fb*tx+b);
    
    dx = 2*dw;
    dy=Aa*sin(2*pi*fa*(tx+dw))-Aa*sin(2*pi*fa*(tx-dw));
    ...
    for k=1:np
        tm=htm\[t(k)+ttraj(i);...
            Aa*sin(2*pi*fa*(t(k)+ttraj(i)));...
            Ab*sin(2*pi*fb*(t(k)+ttraj(i))+b);...
            1];
        xCt(k,:)=tm(1:3);
    end
    
    trajP(:,3*(i-1)+1:3*i)=xCt;
end
...
for i=1:n
    ...
    plot3(trajP(:,3*(i-1)+1),trajP(:,3*(i-1)+2),trajP(:,3*(i-1)+3),'b');
end
hold off
axis equal; grid on;
set(gca,'FontSize',15); set(gca,'xcolor','k');set(gca,'ycolor','k');set(gca,'zcolor','k');
xticks(-0.2:0.2:0.2);xlim([-0.25 0.25]);
yticks(-0.5:0.2:0.5);ylim([-0.5 0.5]) ;
zticks(0:0.2:0.6);
axis([-.3 .3 -.5 .5 0 .75])
